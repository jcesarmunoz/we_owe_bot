...
def get_app():
    ...
    if _app_instance is None:
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Error al crear la aplicación con configuración completa: {e}")
            logger.warning("Creando aplicación mínima. Asegúrate de configurar las variables de entorno.")
...
try:
    ...
    import logging
    logger = logging.getLogger(__name__)
    logger.warning("asgiref no está instalado. Para usar uvicorn, instala: pip install asgiref")
    app = _flask_app
